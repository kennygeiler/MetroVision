...
- **Resolution**: Pin `drizzle-orm` to `^0.45.1` in `package.json` (see AC-14). Prefer the `db.select().from().where()` builder API over `db.query.*` for stability.
- **Prevention**: After any dependency install, run `pnpm why drizzle-orm` and confirm the resolved line matches `^0.45.1`.
