...
This document **logs the intended tuning workflow** so operators (and future app UX) can repeat it: **sample window → human verified cuts → auto detect → metrics → adjust → re-run**. It complements **`eval/runs/STATUS.md`** (canonical baseline, **CEMENTED** production row, improvement tiers) and the in-app **`/tuning`** hub, plus **`eval/runs/README.md`** (append-only ledger) and **`eval/runs/*.json`** (structured run snapshots).

**Execution plan:** GSD **Phase 10** — [`.planning/phases/10-shot-boundary-cut-eval-and-tuning-product/`](../.planning/phases/10-shot-boundary-cut-eval-and-tuning-product/) (`10-RESEARCH.md`, `10-01-PLAN.md`, `10-02-PLAN.md`, `10-VALIDATION.md`). Locked product choices: **global** boundary presets, **human verified cuts editable with history**, **no cost caps v1**, **cut tuning only** (classification unchanged).
...
**Phase 7 (implemented):** **`pnpm eval:boundary-misses -- eval/gold/....json eval/predicted/....json [--tol 0.5] [--json]`** prints every **FN** (unmatched human verified cuts) and **FP** (unmatched pred), same greedy matching as **`eval:pipeline`**. **`evalBoundaryCuts`** also returns **`unmatchedGoldSec`** / **`unmatchedPredSec`** (field names unchanged) for programmatic use.

---

## Immediate next step (Ran / current human verified cuts)
...
2. **TransNet (baseline done 2026-04-10):** Sweeps in **`eval/runs/2026-04-10-transnet-threshold-sweep.md`** — keep TransNet as an **optional** extra-cut source; do not assume it fixes this reference without per-miss analysis.
...
Use the **same** human verified cuts file and **same** source video timebase for every comparison.
...
| **2. Human verified cuts** | Ground-truth cut instants on **film timeline** | **`/eval/gold-annotate`**, export JSON; or `eval/gold/*.json` in repo | **`eval_gold_revisions`** via **`POST /api/eval-gold-revisions`** (append-only history); **`/eval/gold-annotate`** still valid |
...
| **6. Log** | Audit trail for product + support | **`eval/runs/ledger.jsonl`**, **`eval/runs/*.json`**, **`2026-04-10-ran-boundary-timing.md`** | **`boundary_eval_runs`** + human verified cuts revision chain; CLI ledger still optional |
...
**Existing building blocks:** `eval_artifacts` (**`kind`**, **`filmId`**, **`sessionId`**, **`label`**, **`payload`** JSON) — can store human verified / predicted **`cutsSec`** payloads today (`kind` still **`gold`** / **`predicted`**); extend with **`kind=tuning_summary`** or a dedicated table when you add UI.
...
| Piece | Location |
|-------|----------|
| Human verified cuts UI | **`/eval/gold-annotate`** — `src/app/(site)/eval/gold-annotate/page.tsx` |
...
3. **Human verified cuts:** add revisions via **`POST /api/eval-gold-revisions`** or continue using **`/eval/gold-annotate`**; history per film at **`GET /api/eval-gold-revisions?filmId=`**.
...
5. **Score:** **`POST /api/boundary-eval-runs`** with `goldRevisionId` (human verified cuts revision UUID), `presetId`, `predictedCutsSec`, `toleranceSec` — persists P/R/F1 + miss lists. List runs: **`GET /api/boundary-eval-runs?filmId=`**.
...
- **Same clock:** human verified **`cutsSec`** and auto cuts must share **one timebase** (film-absolute from file t=0 or explicit **`timeOffsetSec`**).
- **One knob per run** when learning; log everything so support can replay.
- **Sample first:** users tune on a **clip**; **promote** winning preset to **full-film ingest** without forcing full-length human verified coverage.
...
| 2026-04-11 | **Phase 10** planned: global presets, human verified cuts revision history, eval runs, worker preset path — `.planning/phases/10-shot-boundary-cut-eval-and-tuning-product/`. |
